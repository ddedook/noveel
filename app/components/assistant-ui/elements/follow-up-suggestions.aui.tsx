      ...
      <div className="mx-auto flex min-h-8 w-max items-center gap-2 px-0.5">
        {suggestions.map((suggestion, idx) => (
          <ThreadPrimitive.Suggestion
            key={idx}
            className="aui-thread-followup-suggestion bg-background hover:bg-default/80 rounded-full border px-3 py-1 text-sm whitespace-nowrap transition-colors ease-in"
          ...
          >
            ...
            {suggestion.label && (
              <span className="aui-thread-followup-suggestion-label text-muted ms-1">
                {suggestion.label}
              </span>
            )}
          </ThreadPrimitive.Suggestion>
