...
export type ToolFallbackRootProps = Omit<
  ...
  "isExpanded" | "onExpandedChange" | "defaultExpanded"
> & {
  open?: boolean;
...
function ToolFallbackDuration({
  ...
  return (
    <span
      ...
      className={cn(
        "aui-tool-fallback-duration text-muted text-xs tabular-nums",
        className,
      )}
      {...props}
    >
  ...
  return (
    <Disclosure.Heading>
      <Disclosure.Trigger
        ...
        className={cn(
          "aui-tool-fallback-trigger group/trigger text-muted hover:text-foreground flex w-fit origin-left items-center gap-2 py-1 text-xs transition-[color,transform] active:scale-[0.96]",
          className,
        )}
        {...props}
      >
        ...
        <ChevronDownIcon
          ...
          className={cn(
            ...
            "-rotate-90",
            "group-data-[expanded=true]/trigger:rotate-0",
          )}
        />
      </Disclosure.Trigger>
    </Disclosure.Heading>
...
function ToolFallbackContent({
  ...
  return (
    <Disclosure.Content
      ...
      className={cn(
        "aui-tool-fallback-content group/disclosure-content relative overflow-hidden text-xs outline-none",
    ...
    >
      <div
        className={cn(
          "flex flex-col gap-2 ps-6 pt-1 pb-2",
        )}
      >
        {children}
      </div>
...
function ToolFallbackArgs({
  ...
  return (
    <div
      ...
      {...props}
    >
      <pre className="aui-tool-fallback-args-value bg-default/50 text-foreground/90 rounded-md p-2.5 text-xs whitespace-pre-wrap">
        {argsText}
      </pre>
    </div>
  );
...
function ToolFallbackResult({
  ...
  return (
    <div
      ...
      {...props}
    >
      <p className="aui-tool-fallback-result-header text-muted text-xs font-medium">
        Result:
      </p>
      <pre className="aui-tool-fallback-result-content bg-default/50 text-foreground/90 mt-1 rounded-md p-2.5 text-xs whitespace-pre-wrap">
...
function ToolFallbackError({
  ...
  return (
    <div
      ...
      {...props}
    >
      <p className="aui-tool-fallback-error-header text-muted font-semibold">
        {headerText}
      </p>
      <p className="aui-tool-fallback-error-reason text-muted">
        {errorText}
      </p>
    </div>
  );
  ...
  if (confirming) {
    ...
    return (
      ...
      >
        ...
        {confirmDescription && (
          <p className="aui-tool-fallback-approval-confirm-description text-muted">
            {confirmDescription}
          </p>
        )}
        {confirming.grants && confirming.grants.length > 0 && (
          <ul className="aui-tool-fallback-approval-confirm-grants flex flex-col gap-1">
            {confirming.grants.map((grant) => (
              <li key={grant}>
                <code className="aui-tool-fallback-approval-confirm-grant bg-default rounded px-1.5 py-0.5 text-xs">
                  {grant}
                </code>
              </li>
            ))}
      ...
      >
        {[...allowOptions, ...customOptions, ...rejectOptions].map((option) => (
          <Button
            key={option.id}
            size="sm"
            variant={option === allowOptions[0] ? undefined : "outline"}
            className={pressable}
