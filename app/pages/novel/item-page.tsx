...
export function NovelItemPage() {
  const { novelId } = useNovelRouteContext('item')
  ...
  const listQuery = useEntityList(novelId, 'item')
  ...
  const timelineQuery = useEntityList(
    novelId,
    'itemTimeline',
    selectedId ? { where: { itemId: selectedId } } : { where: { itemId: '__none__' } },
  )

  const { fields: detailFields } = useFormTemplateFields(novelId, 'item')
  const { fields: tlTemplateFields } = useFormTemplateFields(novelId, 'item', 'timeline')
  const { extra: tlExtraFields, content: tlContentField } = useMemo(
    () => splitTimelineExtraFields(tlTemplateFields),
    [tlTemplateFields],
  )

  const items = (listQuery.data ?? []) as Record<string, unknown>[]
  ...
  const filteredItems = useMemo(
    () =>
      filterFlatList(items, filterText, (item) =>
        `${String(item.name ?? '')} ${String(item.owner ?? '')}`,
      ),
    [items, filterText],
  )

  const selectedItem = useMemo(
    () => (selectedId ? items.find((i) => String(i.id) === selectedId) ?? null : null),
    [items, selectedId],
  )

  const { createMutation, updateMutation, deleteMutation } = useEntityMutations(novelId, 'item', {
    onCreated: (id) => {
      setSelectedId(id)
      setActiveTab('detail')
    },
  })
  const tlMutations = useEntityMutations(novelId, 'itemTimeline')

  const treeNodes = useMemo(
    () =>
      buildFlatTree(filteredItems, (item) => (
        <span className="flex min-w-0 flex-col">
          <span className="truncate">{String(item.name ?? '未命名')}</span>
          {String(item.owner ?? '').trim() ? (
            <span className="text-xs text-muted">主人：{String(item.owner).trim()}</span>
          ) : null}
        </span>
      ), {
    ...
    [filteredItems, deleteMutation, selectedId],
  )

  useEffect(() => {
    if (!selectedItem) return
    setDraftName(String(selectedItem.name ?? ''))
    setDraftOwner(String(selectedItem.owner ?? ''))
    setDraftProfile(applyFieldDefaults(asRecord(selectedItem.profile), detailFields))
  }, [selectedItem, detailFields])
  ...
  function handleCreate() {
    ...
    createMutation.mutate({ name, owner: addOwner.trim(), profile: {} })
    setAddOpen(false)
    setAddName('')
    setAddOwner('')
  }
  ...
  function handleSaveDetail() {
    ...
    updateMutation.mutate({
      id: selectedId,
      data: { name: draftName.trim(), owner: draftOwner.trim(), profile: draftProfile },
    })
  }
  ...
  function handleSaveTimeline() {
    ...
    if (tlEditingId) {
      ...
      tlMutations.createMutation.mutate({ itemId: selectedId, ...payload })
    }
    setTlOpen(false)
  }
      ...
      <div className="flex min-h-0 flex-1 gap-4">
        ...
        >
          <WorkspaceTree
            ...
            emptyText={filterText.trim() ? '无匹配物品' : '暂无物品，点击 + 添加'}
          />
        </ManageTreeSidebar>
        ...
        {selectedItem ? (
          ...
          <div className="flex flex-1 flex-col items-center justify-center gap-2 text-muted">
            ...
            <p className="text-sm">选择左侧物品查看详情</p>
          </div>
        )}
      </div>
      ...
      >
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-3">
            <Label>主人</Label>
            <SingleCombobox
              value={addOwner || '__none__'}
              placeholder="选择主人（可选）"
              onValueChange={(v) => setAddOwner(v === '__none__' ? '' : v)}
              options={ownerComboboxOptions}
            />
          </div>
        </div>
