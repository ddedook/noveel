...
function makeResponse(status, res, data) {
	const bodyText = () => data.toString("utf8");
	return {
		...
		// Match Fetch Response: text()/json()/arrayBuffer() must return Promises.
		text: async () => bodyText(),
...
export const SETTINGS_NAMESPACE = settingsNamespace("cursor-subscription");
...
export function apply(ctx, config = {}) {
	const entry = resolveCursorSettings(config);
